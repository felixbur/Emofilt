 ...
 * @author Felix Burkhardt
 */
public class ContourFromStress implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/contourFromStress_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        int tmpRate, sylC = 0, sylNum = u.getSyllables().size();

        for (int syli = 0; syli < sylNum; syli++) {
            Syllable syl = (Syllable) u.getSyllables().elementAt(syli);
            if (syl.isFocusStressed()) { // if the syl has accent
                int step = ((int) rate - 100) / (int) sylC;
                tmpRate = 0;
                for (int tmpSyli = syli - sylC; tmpSyli <= syli; tmpSyli++) {
                    // foreach syl before the accent
                    tmpRate = 100 + (step * (tmpSyli - (syli - sylC)));
                    ((Syllable) u.getSyllables().elementAt(tmpSyli))
                            .changeMeanF0(tmpRate);
