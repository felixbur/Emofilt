 ...
 * @author Felix Burkhardt
 */
public class DurVLFric implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/durVLFric_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            Syllable syl = (Syllable) iter.next();
                syl.changeMannerSpeechRate(rate, Phoneme.fricative_voiceless);
        }
        return u;
    }
