    ...
    public void init(Logger logger, boolean useGui) {
        ...
        try {
            ...
            defaultRate = Integer.parseInt((String) initValues
                    .get("defaultRate"));
            defaultRate2 = Integer.parseInt((String) initValues
                    .get("defaultRate2"));
            if (useGui) {
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        try {
            rate = Integer.parseInt(Util.getValueFromEmotion(emotion, name,
                    type, rateDesignator));
            rate2 = Integer.parseInt(Util.getValueFromEmotion(emotion, name,
                    type, rateDesignator2));
        ...
        u.modelF0Contour();
        Vector tmpF0Vec = new Vector(), indexList = new Vector(), targetList = new Vector();
        int frameCounter = 0;
            ...
            for (Iterator iter2 = syl.getPhonemes().iterator(); iter2.hasNext();) {
                ...
                if (syl.isFocusStressed() && pho.isVowel()) {
                    if (indexList.size() > 1) {
                        ...
                        int lastIndex = ((Integer) indexList.lastElement())
                                .intValue();
                        indexList.add(new Integer(lastIndex
                                + ((actIndex - lastIndex) / 2)));
                        targetList.add(new Integer((int) (u.getF0Mean()
                                * rate2 / 100)));
                    }
                    indexList.add(new Integer(frameCounter + pho.getFrameNum()
                            / 2));
                    targetList.add(new Integer(
                            (int) (syl.getF0Mean() * rate / 100)));
                }
                if (pho.isVoiced()) {
                    for (int i = 0; i < pho.getF0vals().size(); i++) {
                        tmpF0Vec.add(new Integer(((F0Val) pho.getF0vals()
                                .elementAt(i)).getVal()));
                    }
                } else {
        ...
        targetList.add(new Integer((int) ((Syllable) u.getSyllables()
                .lastElement()).getF0Mean()));
        // modify F0Vals
        for (int i = 0; i < indexList.size() - 1; i++) {
            ...
            if (startVal < endVal) {
                ...
                for (int k = startIndex; k < endIndex; k++) {
                    if (((Integer) tmpF0Vec.elementAt(k)).intValue() != 0) {
                        tmpF0Vec.add(k, new Integer(startVal
                                + (int) (vocCounter * step)));
                        vocCounter++;
                    }
                }
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            ...
            for (Iterator iter2 = syl.getPhonemes().iterator(); iter2.hasNext();) {
                ...
                for (int i = 0; i < pho.getFrameNum(); i++) {
                    tmpValVec.add(tmpF0Vec.elementAt(frameCounter));
                    frameCounter++;
                }
                if (pho.isVoiced()) {
