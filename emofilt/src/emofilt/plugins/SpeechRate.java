 ...
 * @author Felix Burkhardt
 */
public class SpeechRate implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/speechRate_init.txt";
...
//        System.out.println("rate: "+rate+", change: "+change+", globRateFac: "+GlobalRateFactor+", globRate: "+globalRate);
        Utterance u = (Utterance) utt.clone();
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            ((Syllable) iter.next()).changeDuration(rate);
        }
        return u;
    }
