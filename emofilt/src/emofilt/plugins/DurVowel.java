 ...
 * @author Felix Burkhardt
 */
public class DurVowel implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/durVowel_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            ...
            syl.changeMannerSpeechRate(rate, Phoneme.short_vowel);
            syl.changeMannerSpeechRate(rate, Phoneme.long_vowel);
        }
        return u;
    }
    ...
    public Element setEmotion(Element emotion) {
        ...
        debugLogger.debug("defaultrate:"+defaultRate);
