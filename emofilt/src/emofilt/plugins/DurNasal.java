 ...
 * @author Felix Burkhardt
 */
public class DurNasal implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/durNasal_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            Syllable syl = (Syllable) iter.next();
                syl.changeMannerSpeechRate(rate, Phoneme.nasal);
        }
        return u;
    }
