 ...
 * @author Felix Burkhardt
 */
public class DurVLStop implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/durVLStop_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            Syllable syl = (Syllable) iter.next();
                syl.changeMannerSpeechRate(rate, Phoneme.stop_voiceless);
        }
        return u;
    }
