 ...
 * @author Felix Burkhardt
 */
public class F0Range implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/f0Range_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            ((Syllable) iter.next()).changeF0Range(rate, utt.getF0MeanOfLastSyllable());
        }
        return u;
    }
