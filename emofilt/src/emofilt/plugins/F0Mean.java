 ...
 * @author Felix Burkhardt
 */
public class F0Mean implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/f0Mean_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            ((Syllable) iter.next()).changeMeanF0(rate);
        }
        debugLogger.debug("meanF0, "+rate+", new mean: "+u.getF0Mean());
        return u;
    }
