 ...
 * @author Felix Burkhardt
 */
public class Variability implements ModificationPlugin {
    private String name = "";

    private String type = "";

    private final String initFileName = "init/variability_init.txt";
    ...
    public Utterance modify(Utterance utt, Element emotion, double globalRate, Language lang) {
        ...
        for (Iterator iter = u.getSyllables().iterator(); iter.hasNext();) {
            ((Syllable) iter.next()).changeVariability(rate);
        }
        return u;
    }
